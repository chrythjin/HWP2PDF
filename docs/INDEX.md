...
- `sessions/20260624_144731_ci-failure-analysis-and-ops-setup.md` - fixed 3 CI failures (Firestore `ignoreUndefinedProperties`, smoke 404/503 issues), granted out-of-band Cloud Tasks Enqueuer + Cloud Run Invoker IAM roles, enabled `cloudtasks.googleapis.com` and created `conversion-queue`, removed dead IAM grant step from deploy workflow, hardened `.gitignore`.
...
- `sessions/20260630_180000_worker-queued-claim-cas.md` - added worker queued-job compare-and-set claim semantics across Memory and Firestore job stores to prevent duplicate conversion on concurrent Cloud Tasks delivery.
