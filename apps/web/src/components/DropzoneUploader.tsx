  ...
  const uploadFile = useCallback(
    async (selectedFile: File) => {
      ...
      try {
        ...
        if (uploadSessionRef.current !== uploadSession) return;
        const completedUpload = JSON.parse(completeText) as UploadResponseWithToken;
        startPolling({
          ...completedUpload,
          accessToken: directUpload.accessToken,
          accessTokenHeader: directUpload.accessTokenHeader,
        });
      } catch (error) {
        if (uploadSessionRef.current !== uploadSession) return;
        setStatus("failed");
